...
└── extension/              -- Chrome extension source (v1.9.41.0+)
    ...
    │       ├── 03-votd-irrelevant-vacancies.md -- VOTD filter root cause
    │       ├── 04-negotiations-dom-analysis.md -- Negotiations page DOM structure
    │       └── 05-chatik-dom-analysis.md  -- Chatik (/chat) DOM analysis
    │
    ├── src/                -- Source modules (built into dist/content.js by esbuild)
    │   ├── content/        -- 7 files: main entry, page handlers, SPA sync
    │   ├── lib/            -- 75 files: selectors, anti-hallucination, storage, timing,
    │   │                   --   rate-limiter, resume-fetch-*, match-scorer-*, quality-*,
    │   │                   --   cover-letter-*, role-implied-skills, derive-skills
    │   ├── parsers/        -- 10 files: vacancy-list, vacancy-detail, resume-detail,
    │   │                   --   negotiations, negotiations-diagnostic, vacancy-diagnostic
    │   ├── engine/         -- 5 files: apply-orchestrator, apply-queue, apply-actions,
    │   │                   --   apply-actions-cover-letter, index
    │   ├── ui/             -- 15 files: shell, sidebar, tabs, tour, panels
    │   └── page-world.js   -- MAIN world script (SPA navigation patching)
    │
    ├── background/         -- Service worker (Manifest V3)
    │   └── index.js        -- onInstalled, onAlarm, onMessage router, badge updates
    │
    ├── popup/              -- Popup UI (redirects to FAB)
    │   └── index.html
    │
...
### 10.1 Path Simplification (v1.9.41.0)

Prior to v1.9.41.0, the extension lived at `hh-extension/hh-auto-respond-extension/`
(two levels of nesting). It is now at `extension/` (zero levels). All path
references in README, AGENT_RULES, verify-docs.json, and the pre-commit hook
were updated. 192 files moved in commit `c70d47f`.
...
- `ahg-rules/no-unicode-graphics` (WARN on em-dash, en-dash, and other Unicode graphics)

The pre-commit hook runs ESLint and **blocks** the commit on any ERROR.
Run manually: `npm run lint` (or `npm run lint:fix`).

### 10.3 Cascade CLI (v1.9.41.0)

`scripts/cascade-task.js` (430 lines, pure Node.js) replaces the old
`cascade-cli.sh` (484 lines bash + jq). Cross-platform, no external dependencies.
Reads `cascade/state.json` (the actual task cascade). The old shell script is
preserved as a 35-line thin wrapper for backward compatibility with any
existing docs or aliases.
...
into 112 ES modules across 5 layers. The decomposition was driven by AHG Rule 12
(anti-monolith: WARN at 200 lines, ERROR at 250 lines, HARD CAP 400 lines) and
the cascade phases P0–P0.5.
...
- `skill-synonyms.js` split from monolith (B3) — synonym map + lookup helpers
- `resume-constants.js` split into 3 files: `resume-constants-core.js`, `resume-constants-title.js`, `resume-constants-visibility.js`
- `resume-fetch.js` decomposed into 18+ submodules under `src/lib/resume-fetch-*.js`
- `match-scorer.js` decomposed into 4 submodules: `match-scorer-skills.js`, `match-scorer-salary.js`, `match-scorer-experience.js`, `match-scorer-title.js`
- `quality-*.js` decomposed into 7 submodules: patterns, flags, recommendations, ats, date-helpers, experience, resume-analyzer

### 11.3 Negotiations Module (v1.9.41.0, F1.4)

`src/parsers/negotiations.js` (240 lines) + `src/parsers/negotiations-diagnostic.js` (194 lines):

- `parseNegotiations()` — main entry, returns array of parsed negotiation objects
- `fetchAndParseNegotiations()` — background fetch + parse, works from any page
- `parseSingleItem(item, idx)` — single-item parser, exported for diagnostic reuse
- `findListContainer(root)` / `findNegotiationItems(root)` — selector-chain helpers
- `diagnoseNegotiationsDOM(opts)` — structured dump following `diagnoseVacancyPage()` pattern

Selector fallback chains (8 keys in `HH_SELECTORS`): primary `data-qa` → relaxed
`data-qa` (`^=` or `~=`) → Bloko BEM class. No Magritte hashed classes (they break
on every deploy). Anti-hallucination guarantees: empty items rejected (no ghost
rows), correct with 50+ items (5 dedicated tests cover 50, 100, item #75, mixed
statuses, unique IDs).
