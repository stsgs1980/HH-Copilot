...
describe("Performance Tests", () => {
  test("should process large number of vacancies efficiently", async () => {
    ...
    for (let i = 0; i < 100; i++) {
      html +=
        '<div class=\\"vacancy-card\\" data-qa=\\"vacancy-serp__vacancy-' +
        i +
        '\\">' +
        '<h3 class=\\"vacancy-title\\">Software Engineer ' +
        i +
        "</h3>" +
        '<span class=\\"vacancy-salary\\">' +
        (100000 + i * 1000) +
        " руб.</span>" +
        '<div class=\\"vacancy-company\\">Company ' +
        i +
        "</div>" +
        '<a href=\\"/vacancy/' +
        i +
        '\\" class=\\"vacancy-title-link\\" data-qa=\\"vacancy-title\\">Link</a>' +
        "</div>";
    }
    html += "</div>";
