...
export function renderNegotiationList() {
  ...
  if (convs.length === 0) {
    const emptyMsg =
      meta.errors && meta.errors.length > 0
        ...
        : "Откликов пока нет";
    list.innerHTML =
      '<div style="padding:24px;text-align:center;font-size:11px;color:#52525b;">' + esc(emptyMsg) + "</div>";
    return;
  }
  ...
  const tabChips = NEGOTIATION_TABS.filter((t) => t.id === "all" || (tabCounts[t.id] || 0) > 0)
    .map((t) => renderTabOriginChip(t.id, tabCounts[t.id] || 0, activeTabFilter === t.id))
    .join("");

  const refreshBtn =
    '<button id="neg-refresh-btn" class="btn btn-outline btn-sm" ' +
  ...
  const filterRow =
    `<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:6px;align-items:center;">${statusChips}${refreshBtn}</div>` +
