...
const PREVIEW_STYLE_NORMAL = "font-size:11px;color:#71717A;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;";
...
export function renderNegotiationItem(c) {
  ...
  const statusBadge =
    '<span style="display:inline-block;font-size:10px;padding:1px 6px;' +
  ...
  const tsSpan = relTime
    ? '<span style="color:#a1a1aa;">·</span><span style="font-size:11px;color:#a1a1aa;">' + esc(relTime) + "</span>"
    : "";

  // -- Raw application date (kept on the company row, as before) --
  const rawDate = c.date ? '<span style="color:#a1a1aa;">·</span><span>' + esc(c.date) + "</span>" : "";

  return (
    '<div class="conv-item" data-conv-id="' +
    esc(c.id) +
