...
export function renderMyResumesPanel() {
  ...
  if (resumes.length > 0) {
    const visibleCount = resumes.filter(
      (r) => (r.visibility || (r.hidden ? "hidden" : "unknown")) === "visible",
    ).length;
  ...
  if (resumes.length === 0) {
    listEl.innerHTML =
      '<div style="padding:8px;text-align:center;">Нажмите "Синхронизировать все" для загрузки резюме</div>';
    return;
  }
  ...
  listEl.innerHTML = resumes
    .map((r, idx) => {
      ...
      if (vis === "hidden") {
        ...
        visBadge =
          '<span class="badge" style="font-size:9px;margin-left:4px;background:#e4e4e7;color:#52525b;">?</span>';
      }
      ...
      const reparseIcon = isActive
        ? '<button class="btn btn-outline btn-sm" data-action="reparse-resume" title="' +
          (vis === "hidden" ? "Перепарсить скрытое" : "Перепарсить") +
          '" ' +
          'style="padding:2px 6px;font-size:13px;line-height:1;' +
          (vis === "hidden" ? "color:#b45309;border-color:#fbbf24;" : "") +
          '">' +
          "(R)</button>"
        : "";
      return (
        '<div class="har-my-resume-item" data-resume-idx="' +
        idx +
        '" style="padding:8px;border-bottom:1px solid #e4e4e7;cursor:pointer;display:flex;align-items:flex-start;gap:8px;' +
...
export function renderResumeListPanel() {
  ...
  container.innerHTML =
    '<div class="har-resume-list-header">Найдено резюме: ' +
    list.length +
    "</div>" +
    list
      .map((r) => {
        const isActive = panelState.resume && panelState.resume.id === r.id;
        return (
          '<div class="har-resume-list-item ' +
          (isActive ? "har-resume-list-active" : "") +
          '">' +
          '<a href="' +
          esc(r.url) +
          '" target="_blank" class="har-resume-list-link">' +
          esc(r.title) +
          "</a>" +
