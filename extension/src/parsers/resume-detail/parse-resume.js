...
import { createLogger, safeGetText } from "../../lib/anti-hallucination.js";
...
import { parseExperience, parseLanguagesAndAbout, parseSkills } from "./parse-resume-sections.js";
...
export function parseResume() {
  ...
  if (resume.visibility === VISIBILITY_UNKNOWN) {
    const hiddenMsg = document.querySelector(
      '[data-qa="resume-hidden-message"], [data-qa*="resume-hidden"], [data-qa="resume-make-visible"], [data-qa*="make-visible"]',
    );
    if (hiddenMsg) {
      resume.visibility = VISIBILITY_HIDDEN;
      resume.hidden = true;
  ...
  resumeLog.info(
    "Skills: " +
      ...
      " | Derived: " +
      (resume.derivedSkills ? resume.derivedSkills.length : 0) +
      " | Experience: " +
