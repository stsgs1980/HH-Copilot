...
export function parseResumeList() {
  ...
  if (allDetected && resumes.length > 0) {
    ...
    resumeLog.info(
      "Strategy 0: " +
        (resumes.length - unknownResumes.length) +
        " detected, " +
        unknownResumes.length +
        " unknown -- trying data-qa cards",
    );
    ...
    for (const sel of RESUME_CARD_SELECTORS) {
      ...
      cards.forEach((card) => {
        ...
        filled++;
        resumeLog.info(
          "  Card: " + cardId.substring(0, 8) + "=" + result.visibility + " (method=" + result.method + ")",
        );
      });
      if (filled > 0) break;
    }
    ...
    if (stillUnknown.length > 0) {
      ...
      stillUnknown.forEach((resume) => {
        ...
        if (card) {
          ...
          resume.hidden = result.hidden;
          resumeLog.info(
            "  Walk: " + resume.id.substring(0, 8) + "=" + result.visibility + " (method=" + result.method + ")",
          );
        }
      });
    }
  ...
  if (stillUnknown.length > 0) {
    resumeLog.info(
      "List visibility: " + stillUnknown.length + " resumes still UNKNOWN -- will be resolved by detail page detection",
    );
  }
