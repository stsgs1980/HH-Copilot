...
export function detectCompany() {
  ...
  if (sideCompany)
    return {
      ...
      value: sideCompany.textContent.trim(),
      tag: sideCompany.tagName,
      href: sideCompany.href || null,
    };
  return { source: null, value: null };
}
...
export function detectBrandedDescription() {
  const branded = document.querySelector(
    '[data-qa="vacancy-branded-description"], .vacancy-branded-description, [class*="branded"]',
  );
  if (branded) {
    return {
