...
export function applyVisibilityFallback(results, visDiag) {
  ...
  if (iframeNotRan.length > 0) {
    fetchLog.info(
      "[VIS-DIAG] Final fallback: " +
        iframeNotRan.length +
        " resumes UNKNOWN (iframe not run) -> defaulting to VISIBLE",
    );
    ...
    iframeNotRan.forEach((r) => {
      fetchLog.info(
        "[VIS-DIAG]   " +
          ...
          '" UNKNOWN->VISIBLE (iframe not run)',
      );
    ...
    iframeRan.forEach((r) => {
      fetchLog.info(
        "[VIS-DIAG]   " +
          ...
          ' "' +
          (r.title || "").substring(0, 30) +
          '" -> UNKNOWN (iframe ran, no indicators found)',
      );
      const diagEntry = visDiag.resumes.find((d) => d.id === r.id);
      if (diagEntry) {
...
export function finalizeVisDiag(results, visDiag) {
  ...
  fetchLog.info(
    "[VIS-DIAG] Total: " +
      visDiag.summary.total +
      ", Visible: " +
      visDiag.summary.visible +
      ", Hidden: " +
      visDiag.summary.hidden +
      ", Unknown: " +
      visDiag.summary.unknown +
      ", Fallbacks: " +
      visDiag.summary.unknownFallbackToVisible,
  );
  results.forEach((r) => {
    fetchLog.info(
      "[VIS-DIAG]   " +
