...
export function extractVisibilityStatus(doc, resumes, html) {
  ...
  if (unknownAfterAll.length > 0) {
    visLog.info(
      "[VIS-DIAG] List: " +
        unknownAfterAll.length +
        " resumes still UNKNOWN -- will be resolved by detail page detection",
    );
    unknownAfterAll.forEach((r) => {
      visLog.info(
        "[VIS-DIAG]   List: " + r.id.substring(0, 8) + ' "' + (r.title || "").substring(0, 30) + '" -> ' + r.visibility,
      );
    });
  }
