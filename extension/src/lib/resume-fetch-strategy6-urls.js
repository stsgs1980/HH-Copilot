...
import { parseExperienceFromExpandedDoc, parseExperienceFromJson } from "./resume-fetch-strategy6-api.js";
...
export function findExpansionUrls(doc, html, resumeId) {
  ...
  allButtons.forEach((btn) => {
    ...
    fetchLog.info(
      'Strategy 6: found expand button: text="' +
        text.substring(0, 50) +
        '" data-qa="' +
        (btn.getAttribute("data-qa") || "") +
        '"' +
        " outerHTML=" +
        btn.outerHTML.substring(0, 200),
    );
  ...
  if (resumeId) {
    ...
    addUrl("https://hh.ru/applicant/resumes/mine/" + resumeId + "/experience", "known-pattern-experience-endpoint");
  }
