...
const REQ_PREFIXES =
  /^(опыт работы с|опыт работы в|опыт с|опыт в|знание|понимание|владение|работа с|работа в|навыки|умение)\s+/i;
...
function isConcreteSentence(s) {
  ...
  const hasVerb =
    /\b(разработ|настрой|оптимиз|поддержк|автоматиз|внедрен|реализ|управляй|управлен|анализ|тест|deploy|build|test|implement|automate|monitor)\w*/i.test(
      s,
    );
