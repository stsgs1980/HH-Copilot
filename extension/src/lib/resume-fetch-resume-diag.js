...
export function logPreParseDiagnostics(html, doc) {
  ...
  fetchLog.info(
    "Pre-parse: " +
      preExpCards.length +
      " company-cards, " +
      preStepperItems.length +
      " stepper-items, " +
      (preShowAll ? preShowAll.length : 0) +
      ' "show all" buttons in HTML',
  );
  ...
  const MONTHS_RE =
    /(?:январ[ьея]|феврал[ьья]|март[ае]?|апрел[ьья]|ма[йия]|июн[ьья]|июл[ьья]|август[ае]?|сентябр[ьья]|октябр[ьья]|ноябр[ьья]|декабр[ьья])\s*\d{4}\s*[\u2013\u2014-]\s*(?:(?:январ[ьея]|феврал[ьья]|март[ае]?|апрел[ьья]|ма[йия]|июн[ьья]|июл[ьья]|август[ае]?|сентябр[ьья]|октябр[ьья]|ноябр[ьья]|декабр[ьья])\s*\d{4}|настоящее\s*время|по\s+настоящее\s+время|сейчас|по\s+сейчас)/gi;
  ...
  const numDateRanges =
    html.match(
      /\d{2}\.\d{4}\s*[\u2013\u2014-]\s*(?:\d{2}\.\d{4}|настоящее\s*время|по\s+настоящее\s*время|сейчас|по\s+сейчас)/gi,
    ) || [];
...
export function resolveVisibilityDecision(resume, pageVis, listMeta, visDiagEntry) {
  ...
  fetchLog.info(
    "[VIS-DIAG] === Visibility decision for " + (resume.id ? resume.id.substring(0, 8) : "unknown") + " ===",
  );
