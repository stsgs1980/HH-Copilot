...
export function runProximitySearch(resumes, html) {
  ...
  if (hashPositions.length > 0) {
    visLog.info(
      "  Hash positions in cleaned HTML: " + hashPositions.map((h) => h.id.substring(0, 8) + "@" + h.pos).join(", "),
    );
  }
  ...
  resumes.forEach((r) => {
    ...
    visLog.info(
      "  " +
        ...
        searchEnd +
        ", next=" +
        (nextResume ? nextResume.id.substring(0, 8) : "none") +
        ", indicators=" +
        (isHidden ? "FOUND" : "none") +
        ")",
    );
