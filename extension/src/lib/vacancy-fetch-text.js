...
import { extractCleanCompanyName, parseExperienceFromDoc } from "./vacancy-fetch-text-helpers.js";
import { parseDescriptionFromDoc, parseKeySkillsFromDoc, parseSalaryFromDoc } from "./vacancy-fetch-text-parsers.js";
...
export function parseVacancyDetailFromDoc(doc, url) {
  ...
  const companyEl = doc.querySelector('[data-qa="vacancy-company-name"], [data-qa="vacancy-company"]');
  if (companyEl) {
  ...
  // -- Location --
  const addrEl = doc.querySelector('[data-qa="vacancy-address-with-map"], [data-qa="vacancy-view-raw-address"]');
  if (addrEl) {
  ...
  // -- Employment type --
  const empEl = doc.querySelector('[data-qa="common-employment-text"], [data-qa*="employment"]');
