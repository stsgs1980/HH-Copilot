...
## Task 4: Version bump 1.9.63.0 → 1.9.49.0 (Rule 9.2, before feat)

**Files:**
- Modify: `extension/manifest.json` (line 5: `"version": "1.9.63.0"`)
- Modify: `extension/package.json` (line 3: `"version": "1.9.63.0"`)
- Modify: `extension/src/lib/version.js` (line 8: `export const VERSION = '1.9.63.0';`)
- Modify: `extension/popup/index.html` (line 28: `v1.9.63.0`)
- Modify: `README.md` (repo root — every `1.9.63.0` occurrence)

**Step 1: Bump all five files**

Replace `1.9.63.0` → `1.9.49.0` in each of the 5 files. In README.md there are multiple occurrences — replace ALL of them, and also update the stale test-count text if it disagrees with the actual suite count (check `npm test` output first; if it's not 349, update the "349 unit tests" / "349 tests" mentions too).
...
git commit -m "chore(F4.1): bump version 1.9.63.0 -> 1.9.49.0"
```
